...
async function sendOTPEmail(email, code, name) {
  ...
  try {
    ...
    console.log(`[EMAIL] ✅ OTP أُرسل إلى ${email}`);
    return { success: true };
  } catch(e) {
...
async function sendResetEmail(email, code, name) {
  ...
  try {
    ...
    console.log(`[EMAIL] ✅ Reset أُرسل إلى ${email}`);
    return { success: true };
  } catch(e) {
...
async function sendActivationEmail(email, name, endDate, subType) {
  ...
  try {
    ...
    console.log(`[EMAIL] ✅ Activation أُرسل إلى ${email}`);
    return { success: true };
  } catch(e) {
    console.error('[EMAIL-ACTIVATION ERROR]', e.message);
    return { success: false, error: e.message };
  }
}
